...
async fn test_full_indexing_flow() {
    ...
    assert_eq!(fetched_doc.content, Some("This is a complete flow test document".to_string()));
    ...
    let _: () = redis_conn.publish("connector_events", update_json).await.unwrap();
    ...
    assert_eq!(final_doc.content, Some("Final content updated via event".to_string()));
    ...
    let _: () = redis_conn.publish("connector_events", delete_json).await.unwrap();
    
    // Wait for deletion and verify
    common::wait_for_document_deleted(&repo, "01JGF7V3E0Y2R1X8P5Q7W9T4N7", "flow_doc_1", Duration::from_secs(5))
        .await
