...
impl SearcherTestFixture {
    ...
    async fn search(&self, query: &str, mode: Option<&str>, limit: Option<u32>) -> Result<(StatusCode, Value)> {
        ...
        let json: Value = serde_json::from_slice(&body).map_err(|e| {
            eprintln!("Failed to parse JSON response. Status: {}, Body: '{}'", status, body_str);
            e
        })?;
...
async fn test_fulltext_search_rust_programming() -> Result<()> {
    ...
    let (status, response) = fixture.search("rust programming", Some("fulltext"), None).await?;
    ...
    let rust_doc = results.iter().find(|doc| {
        doc["document"]["title"].as_str().unwrap_or("").contains("Rust Programming Guide")
    });
...
async fn test_fulltext_search_meeting_planning() -> Result<()> {
    ...
    let (status, response) = fixture.search("meeting planning Q4", Some("fulltext"), None).await?;
    ...
    let meeting_doc = results.iter().find(|doc| {
        doc["document"]["title"].as_str().unwrap_or("").contains("Q4 Planning Meeting")
    });
...
async fn test_fulltext_search_with_title_weight() -> Result<()> {
    ...
    assert!(first_result["document"]["title"].as_str().unwrap_or("").contains("API"));
...
async fn test_semantic_search() -> Result<()> {
    ...
    let (status, response) = fixture.search("software architecture patterns", Some("semantic"), None).await?;
...
async fn test_hybrid_search() -> Result<()> {
    ...
    let (status, response) = fixture.search("rust programming", Some("hybrid"), None).await?;
...
async fn test_search_no_results() -> Result<()> {
    ...
    let (status, response) = fixture.search("nonexistent_query_xyz", Some("fulltext"), None).await?;
...
async fn test_suggestions_endpoint() -> Result<()> {
    ...
    assert!(suggestions.iter().any(|s| 
        s.as_str().unwrap_or("").contains("Rust Programming Guide")
    ));
...
async fn test_cache_behavior() -> Result<()> {
    ...
    assert_eq!(response1["results"].as_array().unwrap().len(), 
...
async fn test_search_highlighting_extraction() -> Result<()> {
    ...
    let (status, response) = fixture.search("memory safety ownership", Some("fulltext"), None).await?;
