...
impl EventProcessor {
    ...
    async fn process_event(&self, payload: &str) -> Result<()> {
        ...
        match event {
            ConnectorEvent::DocumentCreated { 
                ...
                self.handle_document_created(source_id, document_id, content, metadata, permissions).await?;
            }
            ConnectorEvent::DocumentUpdated { 
                ...
                self.handle_document_updated(source_id, document_id, content, metadata, permissions).await?;
            }
            ConnectorEvent::DocumentDeleted { source_id, document_id } => {
    ...
    async fn handle_document_created(
        ...
        info!("Processing document created: {} from source {}", document_id, source_id);
    ...
    async fn handle_document_updated(
        ...
        info!("Processing document updated: {} from source {}", document_id, source_id);
        ...
        if let Some(mut document) = repo.find_by_external_id(&source_id, &document_id).await? {
            ...
            warn!("Document not found for update: {} from source {}", document_id, source_id);
        }
    ...
    async fn handle_document_deleted(
        ...
        info!("Processing document deleted: {} from source {}", document_id, source_id);
        ...
        if let Some(document) = repo.find_by_external_id(&source_id, &document_id).await? {
            ...
            warn!("Document not found for deletion: {} from source {}", document_id, source_id);
        }
